"""Canonical implementation of the public training entry point."""
...
def train(**kwargs):
    ...
    Args for the training:
        ...
        validate_graph_cache: bool. Default=True
                      Validate every cached graph tensor before reuse. Set False only
                      when repeatedly reusing a previously verified cache; recipe,
                      ordered SMILES, and graph-count checks are still performed.
        data_quality_report: bool. Default=True
                      Canonicalize molecules and write duplicate, split-overlap, target,
                      and graph-error diagnostics. Set False for repeated training on
                      data that has already been reviewed; required CSV checks remain.
...
def resolve_config(**kwargs):
    ...
    mark_derived(provenance, config, source="derived_path")
